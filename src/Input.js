...
export function initInput() {
  ...
  canvas.addEventListener("mousedown", (e) => {
    ...
    if (state.player.selectedSkill) {
      const skillUsed = useSkill(
        state.player.selectedSkill,
        gridPos.x,
        gridPos.y,
      );
      if (skillUsed) {
    ...
    const enemy = state.enemies.find(
      (e) => Math.round(e.x) === tX && Math.round(e.y) === tY,
    );
    if (enemy) {
      const now = Date.now();
      ...
      if (
        state.player.equipment.mainhand &&
        state.player.equipment.mainhand.stats
      ) {
        // Check if mainhand has stats (some might not have str explicitly in future)
        if (state.player.equipment.mainhand.stats.str)
          dmg += state.player.equipment.mainhand.stats.str;
      }
      // Basic randomization
      ...
      spawnFloatingText(enemy.x, enemy.y, dmg, isCrit ? "crit" : "damage");

      // Screen shake on hit
      triggerScreenShake(isCrit ? 8 : 4, isCrit ? 300 : 150);
    ...
    const bag = state.lootBags.find(
      (b) => Math.round(b.x) === tX && Math.round(b.y) === tY,
    );
    if (bag) {
      sfx.ui();
