...
import { spawnFloatingText, triggerScreenShake } from "./VisualEffects.js";
...
export function useSkill(skillId, targetX, targetY) {
  ...
  console.log(
    `Used ${skill.name}! MP: ${state.player.mp}/${state.player.maxMp}`,
  );
  return true;
}
...
function executeFireball(targetX, targetY, skill) {
  ...
  state.enemies.forEach((enemy) => {
    ...
    if (dist <= skill.radius) {
      ...
      // Spawn damage number
      spawnFloatingText(enemy.x, enemy.y, damage, "damage");
      hitCount++;
    }
  });

  // Screen shake based on hits
  if (hitCount > 0) {
    triggerScreenShake(5 + hitCount, 200);
  }
...
function executeFrostNova(skill) {
  ...
  state.enemies.forEach((enemy) => {
    ...
    if (dist <= skill.radius) {
      ...
      console.log(
        `Frost Nova hit ${enemy.type} for ${skill.damage} damage + SLOW!`,
      );

      // Spawn damage number
      spawnFloatingText(enemy.x, enemy.y, skill.damage, "damage");
      hitCount++;
    }
  });
...
function executeLightning(targetX, targetY, skill) {
  ...
  targets.forEach((enemy, index) => {
    ...
    console.log(
      `Lightning chain ${index + 1} hit ${enemy.type} for ${chainDamage} damage!`,
    );

    // Spawn damage number
    spawnFloatingText(enemy.x, enemy.y, chainDamage, "damage");
  });

  // Screen shake based on hits
  if (targets.length > 0) {
    triggerScreenShake(4 + targets.length * 2, 200);
  }
