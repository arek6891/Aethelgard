...
import { renderFloatingTexts } from "./VisualEffects.js";
...
function drawTile(screenX, screenY) {
  ...
  if (sprite)
    ctx.drawImage(sprite, Math.floor(screenX - 32), Math.floor(screenY));
}
function drawBlock(screenX, screenY) {
  if (sprites.wall)
    ctx.drawImage(
      sprites.wall,
      Math.floor(screenX - 32),
      Math.floor(screenY - 50),
    );
}
...
function drawEntitySprite(entity) {
  ...
  if (entity.type === "loot") {
    ...
    ctx.drawImage(
      sprite,
      Math.floor(screenX - 16),
      Math.floor(screenY - 10 + bounce),
    );
    return;
  }
  ...
  if (
    entity.type === "player" &&
    state.player.lastAttackTime &&
    now - state.player.lastAttackTime < attackDuration
  ) {
...
export function drawScene() {
  ...
  renderList.push({
    ...
    isMoving:
      Math.abs(state.player.x - state.player.targetX) > 0.01 ||
      Math.abs(state.player.y - state.player.targetY) > 0.01,
    type: "player",
  });
  ...
  if (state.rallyEffects) {
    ...
    for (let i = state.rallyEffects.length - 1; i >= 0; i--) {
      ...
      ctx.ellipse(
        sx,
        sy,
        (radius * config.tileW) / 2,
        (radius * config.tileH) / 2,
        0,
        0,
        Math.PI * 2,
      );
      ...
      ctx.ellipse(
        sx,
        sy,
        ((radius * config.tileW) / 2) * 0.7,
        ((radius * config.tileH) / 2) * 0.7,
        0,
        0,
        Math.PI * 2,
      );
